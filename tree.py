...
def create_tree_parallel(b_array, n, pool):
    ...
    # Parallélise la création des nodes de quadrant (toujours 4, même si certains sont vides)
    nodes = pool.map(create_node_for_quadrant, quadrant_roots)
    ...
    if t.x is not None and t.y is not None and t.L is not None:
        ...
        t.node.subdivide()
        for i in range(4):
            t.node.child[i] = nodes[i]  # même si nodes[i] == None
        t.node.center_of_mass()
    return t